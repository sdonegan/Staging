...
#include "AFRLDistributionC/AFRLDistributionCFilters/util/DifferentialGeometryOps.hpp"
#include "AFRLDistributionC/AFRLDistributionCFilters/util/DistanceTemplate.hpp"
...
void InterpolateMeshToRegularGrid::createRegularGrid()
{
  ...
  image->setResolution(minRes, minRes, minRes);
...
void InterpolateMeshToRegularGrid::dataCheck()
{
  ...
  if(m_ScaleFactorNumCells < 1)
  {
    ...
    notifyErrorMessage(getHumanLabel(), "The scale factor for the total number of Elements must be greater than zero", getErrorCondition());
    return;
  }
...
{
  return "AFRLDistributionC Plugin";
}
